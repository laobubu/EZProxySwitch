 ...
 * @returns {PAC.Result}
 */
var toPACResult = function (values) {
  var retval = [];
...
function autoProxyCvt(rules) {
  ...
  var retval = {
    path_regexs_p: strs2regex(included.contains).map(str => (str)).concat(
      strs2regex(included.startsWith).map(str => ("^(?:" + str + ")")),
      strs2regex(included.endsWith).map(str => ("(?:" + str + ")$"))
    ),
    host_regexs_p: strs2regex(included.domain).map(str => ("^[-\\w\\.]*(?:" + str + ")$")),

    path_regexs_n: strs2regex(excluded.contains).map(str => (str)).concat(
      strs2regex(excluded.startsWith).map(str => ("^(?:" + str + ")")),
      strs2regex(excluded.endsWith).map(str => ("(?:" + str + ")$"))
    ),
    host_regexs_n: strs2regex(excluded.domain).map(str => ("^[-\\w\\.]*(?:" + str + ")$")),
...
!function () {
  ...
  if (FFversion >= 57) {
    toPACResult = function (values) { return values; }
  }
}();
...
function genPACSysConfig(options) {
  ...
  /** @type {PAC.RuleGroupSerializable[]} */
  var ruleGroups = options.ruleGroups
    .filter(rg => rg.enabled)
    .map(ruleGroup => {
      /** @type {PAC.RuleGroupSerializable} */
      var retval = null;

      switch (ruleGroup.ruletype) {
        case 0: retval = autoProxyCvt(ruleGroup.rules); break;
